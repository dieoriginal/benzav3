    ...
    return (
      ...
      >
        <motion.div
          ...
          onDrag={(_, info) =>
            isCarouselActive &&
            rotation.set(rotation.get() + info.offset.x * 0.005) // Reduced sensitivity
          }
          onDragEnd={(_, info) =>
            isCarouselActive &&
            controls.start({
              rotateY: rotation.get() + info.velocity.x * 0.005, // Reduced sensitivity
              transition: {
                type: "spring",
                stiffness: 100,
                damping: 30,
        ...
        >
          {cards.map((imgUrl, i) => (
            ...
            >
              <motion.img
                src="https://imgur.com/07eh8dV"
                alt={`keyword_${i} ${imgUrl}`}
                layoutId={`img-${imgUrl}`}
                className="pointer-events-none w-full object-cover aspect-[4/6.87]"
...
function ThreeDPhotoCarousel() {
  ...
  const cards = [
    "https://m.media-amazon.com/images/I/61UZS3QA1UL._AC_UF894,1000_QL80_.jpg",
