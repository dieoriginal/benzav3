...
export const BadgePro = ({
  ...
  const baseStyle =
    "px-1 py-0.5 rounded-full flex items-center font-extrabold text-[12px] transition-all duration-[0.3s] font-geist"; // Changed font to Geist
  ...
  return (
    <span className={cn(badgeClasses, className)} {...rest}>
      ...
      {children}
      {counter && (
        <small className="flex items-center justify-center w-4 h-4 ml-1 text-xs text-white bg-red-500 rounded-full">
          {counter}
        </small>
      )}
    </span>
